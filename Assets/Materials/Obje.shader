Shader "Custom/LiquidFullControl"
{
    Properties
    {
        _LiquidColor ("Liquid Color (Fallback)", Color) = (1,0,0,1)
        _Color0 ("Color Slice 0 (Bottom)", Color) = (1,0.86,0.09,1)
        _Color1 ("Color Slice 1", Color) = (1,0.18,0.22,1)
        _Color2 ("Color Slice 2", Color) = (0.22,1.0,0.22,1)
        _Color3 ("Color Slice 3 (Top)", Color) = (0.18,0.52,0.95,1)

        // 4 Dilimli Şişe Y Yüksekliği Sınırları (0.06 ile 0.50 arası)
        _Split0 ("Split Y 0-1", Float) = 0.17
        _Split1 ("Split Y 1-2", Float) = 0.28
        _Split2 ("Split Y 2-3", Float) = 0.39
        _SliceCount ("Slice Count", Float) = 1

        _FillAmount ("Fill Amount", Range(-0.1,1.1)) = 0.0
        _Mode ("Mode (0=Y,1=X)", Range(0,1)) = 0

        _TiltX ("Tilt X", Range(-5,5)) = 0
        _TiltZ ("Tilt Z", Range(-5,5)) = 0
        _WobbleStrength ("Wobble Strength", Range(0,0.1)) = 0.0
        _WobbleSpeed ("Wobble Speed", Range(0,10)) = 0.0

        _IsFrozen ("Is Frozen", Float) = 0

        // Renk Canlılığı ve Işıma Ayarları (Canlı, Doygun ve Net İksir Renkleri)
        _ColorBoost ("Color Boost", Range(0.5, 2.5)) = 1.25
        _InnerGlowStrength ("Vibrant Self-Illumination", Range(0.0, 2.0)) = 0.35
        _RimPower ("Rim Power", Range(0.1,8.0)) = 2.0
        _RimIntensity ("Rim Intensity", Range(0, 5.0)) = 0.0
        _HighlightIntensity ("Highlight", Range(0, 3.0)) = 0.0

        _WaveScale ("3D Wave Scale", Range(5.0, 40.0)) = 18.0
        _WaveHeight ("3D Wave Height", Range(0.0, 0.06)) = 0.0
        _TopGlossiness ("Top Surface Glossiness", Range(4.0, 128.0)) = 32.0
    }
    SubShader
    {
        Tags { "RenderType"="Transparent" "Queue"="Transparent" }
        LOD 200
        Blend SrcAlpha OneMinusSrcAlpha
        ZWrite Off
        Cull Off

        CGPROGRAM
        #pragma surface surf Standard alpha:fade fullforwardshadows

        struct Input
        {
            float3 worldPos;
            float3 viewDir;
        };

        fixed4 _LiquidColor;
        fixed4 _Color0;
        fixed4 _Color1;
        fixed4 _Color2;
        fixed4 _Color3;
        float _Split0;
        float _Split1;
        float _Split2;
        float _SliceCount;
        float _IsFrozen;

        float _FillAmount;
        float _Mode;
        float _TiltX;
        float _TiltZ;
        float _WobbleStrength;
        float _WobbleSpeed;
        float _RimPower;
        float _RimIntensity;
        float _HighlightIntensity;
        float _ColorBoost;
        float _InnerGlowStrength;
        float _WaveScale;
        float _WaveHeight;
        float _TopGlossiness;

        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            float3 objPos = mul(unity_WorldToObject, float4(IN.worldPos, 1)).xyz;

            // Sıvı yüzeyi eğim hesabı (Aktarım sırasında çok hafif ve gerçekçi eğim)
            float tilt = objPos.x * _TiltX + objPos.z * _TiltZ;
            float axis = lerp(objPos.y, objPos.x, _Mode) + tilt;
            float freezeFactor = saturate(_IsFrozen);

            if (axis < _FillAmount)
            {
                // 4 katmandan hangisi olduğunu belirle
                fixed4 currentSliceColor = _Color0;
                float sliceBottomY = 0.06;
                float sliceTopY = _Split0;

                if (axis >= _Split2 && _SliceCount >= 4.0)
                {
                    currentSliceColor = _Color3;
                    sliceBottomY = _Split2;
                    sliceTopY = 0.50;
                }
                else if (axis >= _Split1 && _SliceCount >= 3.0)
                {
                    currentSliceColor = _Color2;
                    sliceBottomY = _Split1;
                    sliceTopY = _Split2;
                }
                else if (axis >= _Split0 && _SliceCount >= 2.0)
                {
                    currentSliceColor = _Color1;
                    sliceBottomY = _Split0;
                    sliceTopY = _Split1;
                }

                // Canlı, net ve doygun temel renk (beyazlama veya solukluk yok)
                fixed3 baseColor = currentSliceColor.rgb * _ColorBoost;

                // Renk doygunluğunu güçlendir (renklerin canlı ve parlak patlaması için)
                float gray = dot(baseColor, fixed3(0.299, 0.587, 0.114));
                baseColor = saturate(lerp(fixed3(gray, gray, gray), baseColor, 1.35));

                if (freezeFactor > 0.001)
                {
                    fixed3 frozenTint = lerp(baseColor * 0.7, fixed3(0.75, 0.93, 1.0), 0.50);
                    baseColor = lerp(baseColor, frozenTint, freezeFactor);
                }

                // Katman içi hafif doğal dikey derinlik
                float sliceNormalizedY = saturate((axis - sliceBottomY) / max(0.01, sliceTopY - sliceBottomY));
                float shade = lerp(0.92, 1.05, sliceNormalizedY);
                fixed3 finalColor = baseColor * shade;

                // Katmanlar arasındaki ince ayrım çizgisi (beyaz değil, hafif koyu ton)
                if (_SliceCount > 1.0)
                {
                    float d0 = (_SliceCount >= 2.0) ? abs(axis - _Split0) : 10.0;
                    float d1 = (_SliceCount >= 3.0) ? abs(axis - _Split1) : 10.0;
                    float d2 = (_SliceCount >= 4.0) ? abs(axis - _Split2) : 10.0;
                    float minBoundaryDist = min(d0, min(d1, d2));
                    float boundaryLine = 1.0 - smoothstep(0.001, 0.007, minBoundaryDist);
                    finalColor = lerp(finalColor, finalColor * 0.75, boundaryLine * 0.35);
                }

                o.Albedo = finalColor;
                o.Alpha = 1.0;

                // Kendi renginde canlı ışıldama (Self-Illumination):
                // Beyaz ışık yerine sıvının KENDİ CANLI RENGİ yayılır.
                // Böylece karanlık arka planda asla soluk veya çamurlu durmaz, canlı ve doygun görünür!
                if (freezeFactor > 0.05)
                {
                    float rim = 1.0 - saturate(dot(normalize(IN.viewDir), o.Normal));
                    o.Emission = fixed3(0.7, 0.9, 1.0) * pow(rim, 2.5) * 0.3 * freezeFactor + finalColor * 0.5;
                }
                else
                {
                    o.Emission = finalColor * _InnerGlowStrength;
                }
            }
            else
            {
                o.Alpha = 0;
                o.Albedo = float3(0,0,0);
                o.Emission = float3(0,0,0);
            }

            // Sıvı içi yansımanın dış gökyüzü / skybox tarafından beyazlatılıp soldurulmasını önlemek için 0
            o.Smoothness = 0.0;
            o.Metallic = 0.0;
        }
        ENDCG
    }
    FallBack "Diffuse"
}
